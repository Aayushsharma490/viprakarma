...
function calculateAscendant(date: Date, latitude: number, longitude: number): number {
  try {
    ...
    // Ascendant calculation: tan(asc) = -cos(RAMC) / (sin(RAMC) * sin(lat) + tan(ε) * cos(lat))
    // where ε is the obliquity of the ecliptic (approximately 23.5°)
    const obliquity = 23.5 * Math.PI / 180.0; // obliquity of ecliptic

    const ascendantRad = Math.atan2(
      -Math.cos(ramcRad),
      Math.sin(ramcRad) * Math.sin(latRad) + Math.tan(obliquity) * Math.cos(latRad)
    );
...
function calculateSunLongitude(date: Date): number {
  try {
    ...
    const equationOfCenter = (1.915 * Math.sin(M) + 0.020 * Math.sin(2 * M)) * Math.PI / 180;

    // True longitude
    const trueLongitude = (meanLongitude + equationOfCenter * 180 / Math.PI) % 360;
    ...
    return (280.460 + 0.9856474 * (daysSinceEpoch - 10957)) % 360; // Approximate
  }
}
...
function calculateMoonLongitude(date: Date): number {
  try {
    ...
    const equationOfCenter = (5.128 * Math.sin(M) + 0.541 * Math.sin(2 * M)) * Math.PI / 180;
...
function calculateMercuryLongitude(date: Date): number {
  try {
    ...
    // Mean longitude
    const meanLongitude = (252.251 + 1.602130 * daysSinceJ2000) % 360;
...
function calculateVenusLongitude(date: Date): number {
  try {
    ...
    // Mean longitude
    const meanLongitude = (181.979 + 1.602130 * daysSinceJ2000) % 360;

    // Mean anomaly
    const meanAnomaly = (50.416 + 1.602130 * daysSinceJ2000) % 360;
...
function calculateJupiterLongitude(date: Date): number {
  try {
    ...
    // Mean anomaly
    const meanAnomaly = (20.020 + 0.0831294 * daysSinceJ2000) % 360;
...
function calculateSaturnLongitude(date: Date): number {
  try {
    ...
    // Mean anomaly
    const meanAnomaly = (317.020 + 0.0334597 * daysSinceJ2000) % 360;
...
function calculateRahuKetu(date: Date): { rahu: number; ketu: number } {
  try {
    ...
    // Lunar nodes move retrograde at about 19.3 degrees per year
    // Base position for J2000: Rahu at approximately 125° (Leo)
    const j2000 = new Date(2000, 0, 1, 12, 0, 0);
    const daysSinceJ2000 = (date.getTime() - j2000.getTime()) / (1000 * 60 * 60 * 24);
    const yearsSinceJ2000 = daysSinceJ2000 / 365.25;

    // Rahu moves retrograde, so subtract from base position
    const baseRahuJ2000 = 125; // Approximate Rahu position at J2000
    const rahuLongitude = (baseRahuJ2000 - yearsSinceJ2000 * 19.3) % 360;
    ...
    const daysSinceEpoch = Math.floor(date.getTime() / (1000 * 60 * 60 * 24));
    const rahuApprox = (125 - (daysSinceEpoch - 10957) * 19.3 / 365.25) % 360;
...
export function calculatePlanetaryPositions(
  ...
  try {
    ...
    Object.keys(tropicalPositions).forEach(planet => {
      siderealPositions[planet] = (tropicalPositions[planet] - ayanamsa + 360) % 360;
    });
    ...
    try {
      ...
      console.log('Rahu/Ketu calculated using historical data:', { rahu: rahuLon, ketu: ketuLon });
    } catch (error) {
      ...
      console.log('Using fixed Rahu/Ketu positions:', { rahu: rahuLon, ketu: ketuLon });
    }
    ...
    const result = {
      ...
      mercury: createPosition(siderealPositions.mercury, retrogradeStatus.mercury),
      ...
      jupiter: createPosition(siderealPositions.jupiter, retrogradeStatus.jupiter),
...
function calculateLahiriAyanamsa(date: Date): number {
  ...
  // Lahiri Ayanamsa for J2000 (January 1, 2000 12:00 TT) is 23.853 degrees
  // It increases by approximately 0.01396 degrees per year

  const j2000Ayanamsa = 23.853;
  const annualIncrease = 0.01396; // degrees per year

  // Calculate days since J2000
  const j2000 = new Date(2000, 0, 1, 12, 0, 0); // January 1, 2000 12:00 TT
  const timeDiff = date.getTime() - j2000.getTime();
  const daysSinceJ2000 = timeDiff / (1000 * 60 * 60 * 24);
  const yearsSinceJ2000 = daysSinceJ2000 / 365.25;

  return j2000Ayanamsa + (yearsSinceJ2000 * annualIncrease);
}
