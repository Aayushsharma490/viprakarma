...
import { swissEphemeris, type SwissEphemerisData, type BirthData } from './swissEphemeris';
...
export class VedicAstrologyCalculator {
   ...
   */
  private getZodiacSign(longitude: number): { sign: string; signNumber: number } {
   ...
   */
  private getNakshatra(longitude: number): { nakshatra: string; nakshatraNumber: number; pada: number } {
   ...
   */
  private getHousePlacement(siderealLongitude: number, ascendantLongitude: number): number {
    // Calculate the angular distance from ascendant
    let angle = siderealLongitude - ascendantLongitude;
   ...
   */
  private convertToVedicPlanet(planet: SwissEphemerisData['planets'][0], ayanamsa: number, ascendantLongitude: number): VedicPlanet {
    const siderealLongitude = swissEphemeris.tropicalToSidereal(planet.longitude, ayanamsa);
    const { sign, signNumber } = this.getZodiacSign(siderealLongitude);
    const { nakshatra, nakshatraNumber, pada } = this.getNakshatra(siderealLongitude);
   ...
   * Create Vedic houses
   */
  private createVedicHouses(houses: SwissEphemerisData['houses'], planets: VedicPlanet[]): VedicHouse[] {
   ...
   */
  private createVedicHousesCounterClockwise(ascendantSignNumber: number): VedicHouse[] {
   ...
   */
  async calculateVedicChart(birthData: BirthData, ayanamsaType?: number): Promise<VedicChart> {
    ...
    const { nakshatra: ascendantNakshatra, pada: ascendantPada } = this.getNakshatra(ascendantDegree);
    ...
    const vedicHouses = this.createVedicHousesCounterClockwise(ascendantSignNumber);

    // Assign planets to houses
    vedicHouses.forEach(house => {
      house.planets = vedicPlanets.filter(planet => planet.house === house.number);
    });
  ...
  calculateVimshottariDasha(moonNakshatra: number): Array<{
    ...
    const currentDashaYears = dashaPeriods[moonLord as keyof typeof dashaPeriods];
    const remainingInCurrent = currentDashaYears - (remainingYears % currentDashaYears);

    // Add current dasha
    dashas.push({
      planet: moonLord,
      startDate: new Date(currentDate),
      endDate: new Date(currentDate.getTime() + remainingInCurrent * 365.25 * 24 * 60 * 60 * 1000),
    ...
    for (let i = 1; i < dashaSequence.length && remainingYears > 0; i++) {
      ...
      const years = Math.min(dashaPeriods[planet as keyof typeof dashaPeriods], remainingYears);

      dashas.push({
        planet,
        startDate: new Date(currentDate),
        endDate: new Date(currentDate.getTime() + years * 365.25 * 24 * 60 * 60 * 1000),
        years
      });
  ...
  getPlanetSignifications(planet: VedicPlanet): string[] {
    const significations: { [key: string]: string[] } = {
      'Sun': ['Father', 'Government', 'Authority', 'Health', 'Self-confidence', 'Leadership'],
      'Moon': ['Mother', 'Emotions', 'Mind', 'Home', 'Females', 'Public'],
      'Mars': ['Energy', 'Courage', 'Brothers', 'Property', 'Surgery', 'Enemies'],
      'Mercury': ['Intelligence', 'Communication', 'Business', 'Education', 'Maternal relatives'],
      'Jupiter': ['Wisdom', 'Wealth', 'Children', 'Religion', 'Teaching', 'Foreign lands'],
      'Venus': ['Love', 'Beauty', 'Luxury', 'Spouse', 'Arts', 'Vehicles'],
      'Saturn': ['Discipline', 'Hard work', 'Longevity', 'Servants', 'Justice', 'Delays'],
      'Rahu': ['Ambition', 'Foreign matters', 'Material gains', 'Illusion', 'Technology'],
      'Ketu': ['Spirituality', 'Detachment', 'Past life karma', 'Moksha', 'Healing']
    };
   ...
   */
  private calculatePlanetaryStrength(position: {
    longitude: number;
    latitude: number;
    sign: string;
    degree: number;
  ...
  getHouseSignifications(houseNumber: number): string[] {
    const significations: { [key: number]: string[] } = {
      1: ['Self', 'Personality', 'Physical appearance', 'First impressions', 'Health'],
      ...
      9: ['Higher learning', 'Religion', 'Philosophy', 'Father', 'Foreign travel'],
