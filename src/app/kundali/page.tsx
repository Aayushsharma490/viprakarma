...
interface AstroEngineResponse {
  ...
  dashas: {
    current: { planet: string; startDate: string; endDate: string; years: number };
...
export default function KundaliPage() {
  ...
  const [kundaliData, setKundaliData] = useState<AstroEngineResponse | null>(null);
  const resultsRef = useRef<HTMLDivElement | null>(null);
  const [selectedChart, setSelectedChart] = useState<'d1' | 'chandra' | 'd9' | 'd10'>('d1');
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Unable to generate kundali');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="container mx-auto px-4 py-16">
          <div className="text-center mb-12 relative">
            ...
            <p className="text-lg md:text-xl text-gray-600 mt-4 max-w-3xl mx-auto">
              Enter your birth details to compute an accurate Lagna (D1), Chandra, Navamsa (D9) and
              Dashamsa (D10) chart powered by the Swiss Ephemeris.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="p-6 shadow-lg border border-amber-100">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  <Label className="font-semibold text-gray-900">Date of Birth</Label>
                  <div className="grid grid-cols-3 gap-3 mt-2">
                    <Input
                      placeholder="DD"
                      type="number"
                      value={formData.day}
                      onChange={(event) => setFormData({ ...formData, day: event.target.value })}
                      min={1}
                      max={31}
                      required
                    />
                ...
                <div>
                  <Label className="font-semibold text-gray-900">Time of Birth (24h)</Label>
                  <div className="grid grid-cols-3 gap-3 mt-2">
                    <Input
                      placeholder="HH"
                      type="number"
                    ...
                    <Input
                      ...
                      onChange={(event) => setFormData({ ...formData, second: event.target.value })}
                      min={0}
                      max={59}
                      required
                    />
                ...
                <div className="space-y-2">
                  <Label className="font-semibold text-gray-900">Place of Birth</Label>
                  <Select
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Search within India" />
                    </SelectTrigger>
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <input
                      ...
                      onChange={(event) => setIsManualLocation(event.target.checked)}
                    />
                    <Label htmlFor="manual-location-toggle">Enter location manually</Label>
                  </div>
                </div>

                {isManualLocation && (
                  <div className="space-y-3 rounded-lg border border-dashed border-amber-300 p-3 bg-amber-50/30">
                    <Label className="text-sm font-semibold text-gray-900">City/Town</Label>
                    <Input
                      placeholder="e.g., Jaipur"
                      value={formData.place}
                      onChange={(event) => {
                        const cityName = event.target.value;
                        setFormData({ ...formData, place: cityName });
                ...
                <div>
                  <Label className="font-semibold text-gray-900">Timezone Offset</Label>
                  <Input
                    className="mt-2"
                    placeholder="+05:30"
              ...
              {!kundaliData && (
                <Card className="p-8 h-full flex flex-col justify-center border-dashed border-amber-300">
                  <div className="text-center space-y-3">
                    ...
                    <h3 className="text-2xl font-semibold text-gray-900">Awaiting Birth Details</h3>
                    <p className="text-gray-600">
                      Enter your accurate birth information to unlock high-fidelity Swiss Ephemeris
                      calculations for all major divisional charts.
                    </p>
                  </div>
                </Card>
              )}
                ...
                <div className="space-y-8">
                  ...
                  <Card className="p-6 shadow-sm border border-gray-100">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs uppercase tracking-widest text-gray-500">UTC Time</p>
                          <p className="text-base font-semibold text-gray-900">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-base font-semibold text-gray-900">
                            {kundaliData.ayanamsa.toFixed(6)}°
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center gap-3 text-sm text-gray-700">
                        ...
                        <div>
                          ...
                          <span className="text-gray-500">
                            ({kundaliData.basicDetails.location.latitude.toFixed(4)}°,{' '}
                            {kundaliData.basicDetails.location.longitude.toFixed(4)}°)
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-gray-700">
                        <Compass className="h-4 w-4 text-emerald-600" />
                        <span>Timezone {kundaliData.basicDetails.timezone}</span>
                      </div>
                    </div>
                  </Card>
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap gap-3 justify-center">
                      ...
                      <Button
                        variant={selectedChart === 'chandra' ? 'default' : 'outline'}
                        className={
                      ...
                      <Button
                        variant={selectedChart === 'd10' ? 'default' : 'outline'}
                        className={
                          selectedChart === 'd10'
                    ...
                    {selectedChart === 'd1' && (
                      <Card className="p-4 shadow-sm border border-amber-100">
                        ...
                        <NorthIndianKundali planets={d1Planets} houses={d1Houses} />
                      </Card>
                    )}
                    ...
                    {selectedChart === 'chandra' && (
                      <Card className="p-4 shadow-sm border border-blue-100">
                        ...
                        <ChandraKundaliChart planets={chandraPlanets} houses={chandraHouses} />
                      </Card>
                    )}
                      ...
                      <table className="min-w-full text-sm text-left">
                        ...
                        <tbody>
                          {kundaliData.planets.map((planet) => (
                            ...
                            >
                              <td className="py-2 font-semibold">{planet.name}</td>
                              <td className="py-2">{planet.sign}</td>
                              <td className="py-2">{planet.house}</td>
                              <td className="py-2">{planet.degreeInSign.toFixed(2)}°</td>
                              <td className="py-2">
                      ...
                      <Card className="p-5 shadow-sm border border-gray-100">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          {kundaliData.houses.map((house) => (
                            ...
                            >
                              <span className="text-gray-600">House {house.house}</span>
                              <span className="font-semibold text-gray-900">
                                {house.sign} — {formatDegree(house.cusp)}
                              </span>
                            </div>
                          ))}
                        </div>
                      ...
                      <Card className="p-5 shadow-sm border border-gray-100">
                        ...
                        <div className="space-y-3 text-sm">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold text-gray-900">
                              {kundaliData.nakshatras.sun
                                ? `${kundaliData.nakshatras.sun.name} (Pada ${
                                    kundaliData.nakshatras.sun.pada
                                  })`
                                : '—'}
                            </span>
                          </div>
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold text-gray-900">
                              {kundaliData.nakshatras.moon
                                ? `${kundaliData.nakshatras.moon.name} (Pada ${
                                    kundaliData.nakshatras.moon.pada
                                  })`
                                : '—'}
                            </span>
                          </div>
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold text-gray-900">
                              {kundaliData.nakshatras.ascendant.name} (Pada{' '}
                  ...
                  <Card className="p-6 shadow-sm border border-gray-100">
                    ...
                    <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3">
                      <p className="text-sm font-semibold text-emerald-800">
                        Current Mahadasha — {kundaliData.dashas.current.planet}
                      </p>
                      <p className="text-xs text-emerald-700">
                        {kundaliData.dashas.current.startDate} to{' '}
                        {kundaliData.dashas.current.endDate} (
                        {kundaliData.dashas.current.years.toFixed(2)} years)
                      </p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {kundaliData.dashas.mahadashas.map((dasha) => (
                        ...
                        >
                          <p className="font-semibold text-gray-900">{dasha.planet}</p>
                          <p className="text-gray-600">
                            {dasha.startDate} → {dasha.endDate}
                          </p>
                          <p className="text-xs text-gray-500">
                            Duration: {dasha.years.toFixed(2)} years
                          </p>
                        </div>
                      ))}
                    </div>
