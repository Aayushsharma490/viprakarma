...
import { Star, Clock, Upload, Camera, MessageCircle, Phone, Video, Lock } from 'lucide-react';
...
export default function TalkToAstrologerPage() {
  ...
  const [activeSession, setActiveSession] = useState<{ id: number; astrologerId: number; status: string } | null>(null);
  ...
  const checkChatStatus = async () => {
    try {
      ...
      if (headers) {
        const activeSessionResponse = await fetch('/api/chat/astrologer/start', {
          method: 'GET',
          headers,
        });
  ...
  const handleConnect = (panditId: number, type: 'chat' | 'call' | 'video') => {
    ...
    if (type === 'chat' && activeSession) {
      if (activeSession.astrologerId === panditId && activeSession.status === 'active') {
        router.push(`/chat/astrologer?astrologer=${panditId}`);
        return;
      }

      if (activeSession.status === 'active' && activeSession.astrologerId !== panditId) {
        toast.error('You already have an active chat session. Please complete it before starting another.');
        return;
      }
    }
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="text-center mb-12">
            ...
            {!chatStatus && (
              <div className="mt-4 inline-flex items-center gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-amber-800 text-sm">
                ...
                <span>Chat is locked. Complete a consultation payment to unlock chat.</span>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {astrologers.map((astrologer) => (
              <Card key={astrologer.id} className="bg-white/80 backdrop-blur-sm border-amber-200 hover:shadow-lg transition-shadow">
                <CardHeader className="text-center">
                  ...
                  <CardTitle className="text-xl text-gray-900">{astrologer.name}</CardTitle>
                  <CardDescription className="text-amber-700 font-medium">
                    {Array.isArray(astrologer.specializations) ? astrologer.specializations.join(', ') : astrologer.specializations}
                  </CardDescription>
                  <div className="flex items-center justify-center gap-2 mt-2">
                    <div className="flex items-center">
                      <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                      <span className="ml-1 text-sm font-medium">{astrologer.rating ? astrologer.rating.toFixed(1) : 'New'}</span>
                    </div>
                    <span className="text-gray-400">•</span>
                    <div className="flex items-center">
                      <Clock className="w-4 h-4 text-gray-400" />
                      <span className="ml-1 text-sm">{astrologer.experience} years</span>
                    </div>
                  </div>
                </CardHeader>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {Array.isArray(astrologer.specializations) && astrologer.specializations.slice(0, 3).map((spec: string, index: number) => (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-sm text-gray-600">
                        <span className="font-medium">₹{astrologer.hourlyRate}/hr</span>
                      </div>
                      <Badge variant={astrologer.isOnline ? "default" : "secondary"}>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200 space-y-2">
                      <div className="flex gap-2">
                        <Button
                          ...
                          className={`flex-1 ${activeSession && activeSession.astrologerId === astrologer.id && activeSession.status === 'active' ? 'bg-green-600 hover:bg-green-700' : (chatStatus ? 'bg-blue-600 hover:bg-blue-700' : 'bg-red-600 hover:bg-red-700')} text-white`}
                          size="sm"
                          disabled={
                            // Disable if chat is not enabled OR session is not active for this astrologer
                            !chatStatus || !(activeSession && activeSession.astrologerId === astrologer.id && activeSession.status === 'active')
                          }
                        >
                          {activeSession && activeSession.astrologerId === astrologer.id && activeSession.status === 'active' ? <MessageCircle className="w-4 h-4 mr-1" /> : (chatStatus ? <MessageCircle className="w-4 h-4 mr-1" /> : <Lock className="w-4 h-4 mr-1" />)}
                          {activeSession && activeSession.astrologerId === astrologer.id && activeSession.status === 'active' ? 'Continue Chat' : (chatStatus ? 'Chat Now' : 'Start Chat (Locked)')}
                        </Button>
                        <Button
