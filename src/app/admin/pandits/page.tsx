...
export default function AdminPanditsPage() {
  ...
  const handleToggleApproval = async (id: number, currentStatus: boolean) => {
    ...
    try {
      const response = await fetch(`/api/admin/pandits/${id}/approve`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        throw new Error('Failed to update pandit status');
      }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      required={!editingPandit}
                      placeholder={editingPandit ? "Leave empty to keep current password" : ""}
                    />
                  </div>
                  <div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`px-2 py-1 text-xs rounded ${pandit.isApproved ? 'bg-blue-500/20 text-blue-500' : 'bg-yellow-500/20 text-yellow-500'}`}>
                          {pandit.isApproved ? 'Approved' : 'Pending'}
                        </span>
                      </div>
                      ...
                      {pandit.email && (
                        <p className="text-sm text-muted-foreground mt-1">Email: {pandit.email}</p>
                      )}
                      {pandit.description && (
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <Button
                        size="sm"
                        variant={pandit.isApproved ? "destructive" : "default"}
                        onClick={() => handleToggleApproval(pandit.id, pandit.isApproved)}
                        disabled={processingAction === `toggle-${pandit.id}`}
                      >
                        {processingAction === `toggle-${pandit.id}` ? <Loader2 className="w-4 h-4 animate-spin" /> : (pandit.isApproved ? 'Disapprove' : 'Approve')}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
