    ...
    if (openai) {
      try {
        const messages = [
          {
            role: 'system',
            content: 'You are an expert Vedic astrologer with deep knowledge of astrology, numerology, palmistry, and ancient Indian wisdom. Provide insightful, compassionate, and culturally sensitive guidance. Include relevant Sanskrit terms and traditional concepts when appropriate. Always be respectful and encouraging.'
          },
        ...
        responseMessage = completion.choices[0]?.message?.content || 'I apologize, but I could not generate a response. Please try again.';
      } catch (error) {
