...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !email || !password || !phone || !specializations || !experience || !hourlyRate) {
      return NextResponse.json({ 
        error: "Missing required fields: name, email, password, phone, specializations, experience, hourlyRate",
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete astrologer
    const deleted = await db.delete(astrologers)
      .where(eq(astrologers.id, parseInt(id)))
      .returning();
